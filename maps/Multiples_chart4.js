			...
			var colorDensity = d3.scale.quantize()
			...
			//Create SVG element
			var svgDensity = d3.select("#az-countyDensity-map")
						.append("svg")
						.attr("width", w)
						.attr("height", h);
			...
			d3.csv("data/arizona-counties.csv", function(data) {
				...
				colorDensity.domain([
					...
					d3.max(data, function(d) { return 500; })
				]);
				...
				d3.json("data/us-arizona-counties.json", function(json) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						var dataValue = parseFloat(data[i].peoplePerSqMi);
					...
					svgDensity.selectAll("#az-countyDensity-map")
					   .data(json.features)
					   .enter()
					   .append("path")
					   .attr("d", path)
					   ...
					   .on("mouseover", function(d) {   //Add tooltip on mouseover for each circle
								...
								d3.select("#tooltip4")
									...
									.select("#countyDensity-label")	
									//CSV data has been bound to JSON at this point - so values must be referenced from JSON properties
									.html("<strong>" + d.properties.name + "</strong>" + "<br/>" + "People per Sq. Mile: " + d.properties.value)			

			...
			var colorFirmsWomen = d3.scale.quantize()
			...
			//Create SVG element
			var svgFirmsWomen = d3.select("#az-countyFirmsWomen-map")
						.append("svg")
						.attr("width", w)
						.attr("height", h);
			...
			d3.csv("data/arizona-counties.csv", function(data) {
				...
				colorFirmsWomen.domain([
					d3.min(data, function(d) { return 0; }), 
					d3.max(data, function(d) { return 50; })
				]);
				...
				d3.json("data/us-arizona-counties.json", function(json) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						var dataValue = parseFloat(data[i].noFirmsWomen);
					...
					svgFirmsWomen.selectAll("#az-countyFirmsWomen-map")
					   .data(json.features)
					   .enter()
					   .append("path")
					   .attr("d", path)

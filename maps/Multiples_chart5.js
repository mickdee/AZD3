			...
			var colorFirms = d3.scale.quantize()
			...
			//Create SVG element
			var svgFirms = d3.select("#az-countyFirms-map")
						.append("svg")
						.attr("width", w)
						.attr("height", h);
			...
			d3.csv("data/arizona-counties.csv", function(data) {
				...
				colorFirms.domain([
					...
					d3.max(data, function(d) { return 111765; })
				]);
				...
				d3.json("data/us-arizona-counties.json", function(json) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						var dataValue = parseFloat(data[i].noFirms);
					...
					svgFirms.selectAll("#az-countyFirms-map")
					   .data(json.features)
					   .enter()
					   .append("path")
					   .attr("d", path)
					   ...
					   .on("mouseover", function(d) {   //Add tooltip on mouseover for each circle
								...
								d3.select("#tooltip5")
									...
									.select("#countyFirms-label")	
									//CSV data has been bound to JSON at this point - so values must be referenced from JSON properties
									.html("<strong>" + d.properties.name + "</strong>" + "<br/>" + "Number of Firms: " + d.properties.value)			
